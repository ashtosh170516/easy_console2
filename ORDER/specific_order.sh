...
order_details () {
    ...
    echo "<br>"
    echo '<pre>'
    sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript order_node pretty_print "[<<\"$1\">>]."
    echo '</pre>'
    echo "<br>"
    echo "<br>"
    echo "<br>"
    ...
    echo "<br>"
    echo '<pre>'
    sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript order_node get_by_id "[<<\"$1\">>]."
    echo '</pre>'
    echo "<br>"
}
...
echo '<title>specific Order details</title>' 
...
echo '<span class="details"style="margin-right:-11px">Enter ORDER_ID</span>' 
echo '<input type="number" name="ORDER_ID" size=12 placeholder="ORDER_ID" required>' 
echo '</div>' 
