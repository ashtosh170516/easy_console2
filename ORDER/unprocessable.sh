...
all_unprocessable_order() {
    ...
    if [ "$1" -eq "1" ]; then
       echo '<pre>' 
       sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript order_node search_by "[[{'status', 'equal','unprocessable'}], 'key']."
       echo '</pre>'
    elif [ "$1" -eq "2" ]; then
       echo '<pre>'
       sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript order_node search_by "[[{'status', 'equal', 'unprocessable'}], 'record']."
       echo '</pre>'
    else
...
echo '<title>unprocessable_orders</title>'
...
echo '<input type="number" name="Input_key" size=12 placeholder="Enter 1 or 2" required>'
echo '</div>'
