...
order_notification () {
    ...
    echo "<br>"
    echo '<pre>'
    sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript station_recovery send_notification "[{'order_notification',[<<\"$1\">>]}]."
    echo '</pre>'
...
echo '<span class="details">Enter ORDER_ID</span>' 
echo '<input type="number" name="ORDER_ID" style="padding-right:6px" size=12 placeholder="ORDER_ID" required>' 
echo '</div>' 
