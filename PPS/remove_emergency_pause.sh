#!/bin/bash
remove_emergency_pause () {
    echo "Removing emergency pause for Controller Id : <<'$1'>> "
    echo "<br>"
    echo '<pre>'
    sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript generic_peripheral remove_emergency_pause "[<<\"$1\">>]."
    echo '</pre>'
...
echo '<title>Remove emergency_pause</title>' 
...
echo '<span class="details">Enter CONTROLLER_ID</span>' 
echo '<input type="number" name="CONTROLLER_ID" style="padding-right:6px" size=12 placeholder="CONTROLLER_ID" required>' 
echo '</div>' 
