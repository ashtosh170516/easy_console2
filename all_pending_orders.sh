...
all_pending_order () {
    ...
    if [ "$1" -eq "1" ]; then
       echo '<pre>'
       sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript order_node search_by "[[{'status', 'equal','pending'}], 'key']."
       echo '</pre>'
    elif [ "$1" -eq "2" ]; then
       echo '<pre>'
       sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript order_node search_by "[[{'status', 'equal', 'pending'}], 'record']."
       echo '</pre>'
    else
