...
butler_associated_to_task () {
    ...
    if [ "$2" -eq "1" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript butlerinfo search_by "[[{'taskkey', 'equal', <<\"$1\">>}], 'key']."
       echo '</pre>'
    elif [ "$2" -eq "2" ]; then
      echo '<pre>'
...
echo '<input type="text" name="Task_key" style="padding-right:6px" size=12 placeholder="Task_key" required>' 
echo '<span class="details">Type 1 for key and 2 for record:</span>' 
echo '<input type="text" name="Type 1 for key and 2 for record:" style="padding-right:6px" size=12 placeholder="1 or 2" required>' 
echo '</div>' 
