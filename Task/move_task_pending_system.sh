...
move_task_pending_system () {
    ...
    if [ "$1" -eq "1" ]; then
       echo '<pre>'
       sudo /opt/butler_server/erts-9.3.3.6/bin/escript /usr/lib/cgi-bin/rpc_call.escript movetaskrec search_by "[[{'status', 'notequal', 'complete'}], 'key']."
       echo '</pre>'
    elif [ "$1" -eq "2" ]; then
      echo '<pre>'
       sudo /opt/butler_server/erts-9.3.3.6/bin/escript /usr/lib/cgi-bin/rpc_call.escript movetaskrec search_by "[[{'status', 'notequal', 'complete'}], 'record']."
       echo '</pre>'
    else 
