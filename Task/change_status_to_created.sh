...
change_status_to_created () {
    ...
    echo "<br>"
    if [ "$2" -eq "1" ]; then
        sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript butler_task_functions set_task_status "[{'picktask',<<\"$1\">>},{'pending','created'},'undefined']."
    elif [ "$2" -eq "2" ]; then
        sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript butler_task_functions set_task_status "[{'audittask',<<\"$1\">>},{'pending','created'},'undefined']."
    else
