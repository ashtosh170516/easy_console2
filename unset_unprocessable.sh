	#!/bin/bash
unset_unprocessable () {
    echo "unsetting unprocessable Order_ID : <<'$1'>>"
    echo "<br>"
    echo '<pre>'
    sudo /opt/butler_server/erts-11.1.1/bin/escript /usr/lib/cgi-bin/rpc_call.escript station_recovery unset_unprocessable "[{'order',<<\"$1\">>}]."
    echo '</pre>'
...
echo '<title>Unset unprocessable orders</title>' 
...
echo '<span class="details"style="margin-right:-26px">Enter Unprocessable ORDER_ID</span>' 
